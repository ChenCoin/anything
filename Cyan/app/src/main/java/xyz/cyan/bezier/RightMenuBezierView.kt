...
class RightMenuBezierView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        path.reset()
        path.moveTo(theWidth, touchY - effectSize / 2)
        path.cubicTo(
            theWidth, touchY - effectSize / 4,
            theWidth + touchX, touchY - effectSize / 4,
            theWidth + touchX, touchY
        )
        path.cubicTo(
            theWidth + touchX, touchY + effectSize / 4,
            theWidth, touchY + effectSize / 4,
            theWidth, touchY + effectSize / 2
        )
    ...
    private fun onMove(event: MotionEvent, x: Float): Boolean {
        ...
        if (event.action == MotionEvent.ACTION_MOVE) {
            touchX = x.coerceAtLeast(-maxEffect)
            runMenuEvent(x)
        }
